...
const Skills = () => {
    return (
        <section className="py-20 bg-white dark:bg-gray-800" id="skills">
            <h2 className="text-center text-3xl md:text-5xl font-bold mb-5">Skills</h2>
            <div className="flex flex-wrap justify-center gap-4 px-5 md:px-[400px]">
                {skills.map((skill, index) => (
                    <div key={index} className="dark:bg-gray-700 bg-white shadow-lg dark:text-white text-gray-900 px-4 py-2 ">{skill}</div>
                ))}
            </div>
        </section>
    );
